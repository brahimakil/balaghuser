...
import { Calendar, Clock, Eye, Radio, Newspaper } from 'lucide-react';
...
const News: React.FC = () => {
  ...
  const [pressNews, setPressNews] = useState<NewsItem[]>([]);
  ...
  useEffect(() => {
    const fetchNews = async () => {
      try {
        ...
        // Fetch all news and separate into different categories
        const [allNews, liveNewsData] = await Promise.all([
          getAllNews(),
          getLiveNews()
        ]);

        // Separate press news and regular news from non-live news
        const pressNewsData = allNews.filter(news => 
          news.type === 'regular' && news.isPressNews === true
        );
        
        const regularNewsData = allNews.filter(news => 
          news.type === 'regular' && news.isPressNews !== true
        );

        setLiveNews(liveNewsData);
        setPressNews(pressNewsData);
    ...
    return (
      ...
      >
        ...
        <div className="relative h-48 bg-primary-100 dark:bg-primary-700">
          ...
          {isPress && (
            <div className="absolute top-3 right-3 flex items-center space-x-1 bg-blue-600 text-white px-3 py-1 rounded-full">
              <Newspaper className="h-3 w-3" />
              <span className="text-xs font-bold">{language === 'ar' ? 'صحفي' : 'PRESS'}</span>
            </div>
          )}
        </div>
      ...
      <div className="container mx-auto px-4 sm:px-6 lg:px-8 py-8 space-y-12">
          ...
          <div>
            <div className="flex items-center space-x-3 mb-6">
              <div className="flex items-center space-x-2 bg-blue-600 text-white px-4 py-2 rounded-full">
                ...
                <span className="font-bold">{language === 'ar' ? 'أخبار صحفية' : 'PRESS NEWS'}</span>
              </div>
              <h2 className="text-2xl font-bold text-primary-900 dark:text-white">
                {language === 'ar' ? 'الأخبار الصحفية' : 'Press News'}
              </h2>
              <span className="text-sm text-primary-500 dark:text-primary-400">
                ({pressNews.length} {language === 'ar' ? 'خبر صحفي' : 'press news'})
              </span>
            </div>

            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
              {pressNews.map((news) => renderNewsCard(news, false, true))}
            </div>
          </div>
        )}
        ...
        <div>
          ...
          {regularNews.length === 0 ? (
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
              {regularNews.map((news) => renderNewsCard(news, false, false))}
            </div>
          )}
        </div>
      </div>
